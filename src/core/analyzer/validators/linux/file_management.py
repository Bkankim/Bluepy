...
def check_u16(command_outputs: List[str]) -> CheckResult:
    ...
    for line in lines:
        if '.' in line:
            return CheckResult(
                status=Status.FAIL,
                message=f"취약: PATH에 '.'이 포함되어 있습니다 - {line}"
            )

    return CheckResult(
        status=Status.PASS,
        message="안전: PATH에 '.'이 포함되어 있지 않습니다"
    )
...
def check_u17(command_outputs: List[str]) -> CheckResult:
    ...
    if not lines0 and not lines1:
        return CheckResult(
            status=Status.PASS,
            message="안전: 소유자가 존재하지 않는 파일이 없습니다"
        )
    else:
        return CheckResult(
            status=Status.FAIL,
            message="취약: 소유자가 존재하지 않는 파일이 발견되었습니다"
        )
...
def check_u28(command_outputs: List[str]) -> CheckResult:
    ...
    if all_empty:
        return CheckResult(
            status=Status.PASS,
            message="안전: .rhosts 또는 hosts.equiv 파일이 없습니다"
        )
    ...
    for line in lines:
        ...
        if 'umask' in stripped:
            ...
            if len(parts) >= 2 and parts[0] == 'umask' and parts[1] == '022':
                return CheckResult(
                    status=Status.PASS,
                    message="안전: umask 022 설정이 존재합니다"
                )

    return CheckResult(
        status=Status.FAIL,
        message="취약: umask 022 설정이 없습니다"
    )
